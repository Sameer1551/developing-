...
const NortheastMap: React.FC<NortheastMapProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    const loadMapData = async () => {
      try {
        ...
        const states = [...new Set(districtGeoJSON.features.map((f: any) => f.properties.NAME_1))];
  ...
  const onEachFeature = (feature: MapFeature, layer: any) => {
    layer.on({
      click: () => {
    ...
    <div className={`flex ${className}`}>
      ...
      <div className="flex-1">
        ...
        >
          ...
          <GeoJSON
            data={{ type: 'FeatureCollection', features: stateData } as any}
            style={getStateStyle}
            key="state-borders"
          />
          ...
          <GeoJSON
            data={{ type: 'FeatureCollection', features: districtData } as any}
